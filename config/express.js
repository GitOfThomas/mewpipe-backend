...
var verifyHandler = function(token, tokenSecret, profile, done) {
  process.nextTick(function() {
    ...
    Users.findOne({uid: profile.id}, function(err, user) {
      if (user) {
        ...
        if(data.provider === 'twitter') {
          data.profile_img_url = profile.photos[0].value;
        }
        if(data.provider === 'google') {
          data.profile_img_url = profile._json.picture;
        }
        if(data.provider === 'facebook') {
          if(data.lastname !== undefined && data.firstname !== undefined) {
            data.profile_img_url = 'https://graph.facebook.com/' + data.firstname + data.lastname + '/picture'
          }
        }
        ...
        if(data.provider === 'github') {
          ...
          data.profile_img_url = profile._json.avatar_url;
        }
...
var verifyHandlerOpenID = function(identifier, profile, done) {
  process.nextTick(function() {

    Users.findOne({uid: identifier}, function(err, user) {
      if (user) {
...
module.exports.http = {
  ...
  customMiddleware: function(app) {
    ...
    passport.use(new OpenidStrategy({
      returnURL: serverUrl + 'users/openid/callback',
      realm: serverUrl,
      profile: true
    }, verifyHandlerOpenID));
